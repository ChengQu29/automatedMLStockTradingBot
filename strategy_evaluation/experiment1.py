...
if __name__=="__main__":
    ...
    benchmark_order = mst.benchmark_orders(symbol_to_test, in_sample_sd, in_sample_ed, sv)
    ...
    manual_orders = mst.testPolicy(symbol_to_test, in_sample_sd, in_sample_ed, sv)

    manual_vals=compute_portvals(manual_orders, sv, 0.0, 0.0)
    normed_manual_vals = manual_vals/manual_vals[0]

    strl = strl.StrategyLearner()
    strl.add_evidence(symbol="JPM", sd=in_sample_sd, ed=in_sample_ed, sv=10000)
    trades= strl.testPolicy(symbol="JPM", sd=in_sample_sd, ed=in_sample_ed, sv=10000)
    strategy_orders=strl.generate_marketsim_orders(symbol_to_test, trades)
    ...
    benchmark_order = mst.benchmark_orders(symbol_to_test, out_sample_sd, out_sample_ed, sv)

    bench_vals = compute_portvals(benchmark_order, sv, 0.0, 0.0)
    normed_benchmark_vals = bench_vals / bench_vals[0]

    # generage manual strategy
    manual_orders = mst.testPolicy(symbol_to_test, out_sample_sd, out_sample_ed, sv)

    manual_vals = compute_portvals(manual_orders, sv, 0.0, 0.0)
    normed_manual_vals = manual_vals / manual_vals[0]


    strl.add_evidence(symbol="JPM", sd=out_sample_sd, ed=out_sample_ed, sv=10000)
    trades = strl.testPolicy(symbol="JPM", sd=out_sample_sd, ed=out_sample_ed, sv=10000)
    strategy_orders = strl.generate_marketsim_orders(symbol_to_test, trades)
    strategy_vals = compute_portvals(strategy_orders, sv, 0.0, 0.0)
    normed_strategy_vals = strategy_vals / strategy_vals[0]
